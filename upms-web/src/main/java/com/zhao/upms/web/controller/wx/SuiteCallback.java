...
public class SuiteCallback {
     ...
     * @throws IOException
     */
    @RequestMapping(value = "/{suiteid}")
    public String acceptAuthorizeEvent(@PathVariable String suiteid, HttpServletRequest request, HttpServletResponse response) throws IOException, AesException,
            DocumentException {
        logger.debug("获取到的第三方应用suiteid:" + suiteid);
        return processAuthorizeEvent(suiteid, request, response);
    }
     ...
     */
    public String processAuthorizeEvent(String suitedid, HttpServletRequest request, HttpServletResponse response) throws IOException,
            DocumentException, AesException {        // 第三方事件回调
        WXBizMsgCrypt wxcpt = new WXBizMsgCrypt(WxCpApiPathConsts.token, WxCpApiPathConsts.encodingAESKey, suitedid);        // 解析出url上的参数值如下：
